...
class ControllerExtensionPaymentPaymenterio extends Controller {
    ...
    public function index() {
        ...
        $this->language->load('extension/payment/paymenterio');

		$orderInfo = $this->model_checkout_order->getOrder($this->session->data['order_id']);
        $orderID = $this->session->data['order_id'];

        $shop_id = $this->config->get('payment_paymenterio_shop_id');
        $api_key = $this->config->get('payment_paymenterio_api_key');

        $urls = $this->getReturnUrlsForOrder($orderID, $orderInfo['email']);
        $shop = new Shop($shop_id, $api_key);

        try {
            $paymentData = $shop->createPayment(
                1,
                $orderID,
                $this->getAmountForOrder($orderInfo['total'], $orderInfo['currency_code']),
        ...
        $data['payment_link'] = $paymentData->payment_link;
        $data['redirect_info'] = $this->language->get('redirect_info');

		try{
			$order_info = $this->model_checkout_order->getOrder($orderID);
			if($order_info['order_status_id'] != $this->config->get('payment_paymenterio_order_status_new_id')){
				$message = 'Rozpoczęcie płatności dla zamówienia o nr. '.$orderID.' w serwisie Paymenterio';
				$this->model_checkout_order->addOrderHistory($orderID, $this->config->get('payment_paymenterio_order_status_new_id'), $message, false);
			}
        ...
        return $this->load->view('extension/payment/paymenterio', $data);
    }
    ...
    public function callback() {
        ...
        $orderNewStatusID = $this->config->get('payment_paymenterio_order_status_new_id');
        $orderCancelStatusID = $this->config->get('payment_paymenterio_order_status_cancel_id');
        $orderProcessedStatusID = $this->config->get('payment_paymenterio_order_status_id');
        ...
        $isSignatureValid = SignatureGenerator::verifySHA1Signature($orderID, $orderInfo['email'], $hash);
        if (!$isSignatureValid) {
            Header('HTTP/1.1 400 Bad Request');
        ...
        if ($currentOrderStatus != $orderProcessedStatusID) {
            if ($statusID == 5) {
                $message = str_replace('{{hash}}', $body['transaction_hash'], $this->language->get('payment_success'));
                $this->model_checkout_order->addOrderHistory($orderID, $orderProcessedStatusID, $message, false);
                exit('Success');
            } elseif ($statusID <= 4) {
                $message = str_replace('{{status}}', $statusID, $this->language->get('payment_status_changed'));
                $this->model_checkout_order->addOrderHistory($orderID, $orderNewStatusID, $message, false);
                exit ('Changed');
            } else {
                $message = str_replace('{{status}}', $statusID, $this->language->get('payment_cancelled'));
                $this->model_checkout_order->addOrderHistory($orderID, $orderCancelStatusID, $message, false);
                exit('Cancelled');
            }
        }
    ...
    private function getAmountForOrder($total, $currency)
    {
        return array(
            "value"=>$total,
    ...
    private function getNameForOrder($orderID) {
        return "Płatność za zamówienie {$orderID}";
    }

    private function getReturnUrlsForOrder($orderID, $orderEmail)
    {
        return array(
            'successUrl' =>  $this->url->link('checkout/success'),
            'failUrl' => $this->url->link('checkout/failure'),
            'notifyUrl' => $this->buildNotifyUrl($orderID, $orderEmail)
        );
    }

    private function buildNotifyUrl($orderID, $orderEmail) {
        $actionURL = $this->url->link('extension/payment/paymenterio/callback');
